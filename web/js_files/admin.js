/**
 * @file admin.js
 * @description Yönetici (Admin) panelinin tüm iş mantığını içerir. Kullanıcı ekleme,
 * silme, yetkilendirme ve genel istatistikleri (dashboard) çekme işlemleri burada yapılır.
 * Sadece 'admin' rolüne sahip kullanıcılar bu fonksiyonaliteye erişebilir.
 */
...
// =====================
/**
 * Sistem istatistiklerini (Toplam kullanıcı, aktif kullanıcı vb.) ve 
 * son eklenen kullanıcıları (Recent Users) `users` koleksiyonundan çeker.
 * Mesaj istatistiklerini de `messages` koleksiyonundan getirir.
 */
...
/**
 * Veritabanındaki tüm kullanıcıları (admin hariç tutularak frontend tarafında)
 * oluşturulma tarihine (createdAt) göre tersten sıralı çeker ve listeye atar.
 */
...
/**
 * Hafızaya alınmış kullanıcı listesi (`allUsersData`) üzerinde anlık (client-side)
 * filtreleme (Arama çubuğu, kategori sekmesi, bölge filtresi) yapar ve HTML tablosunu çizer.
 */
...
/**
 * "Yeni Personel Kaydı" formunda; isim, şirket ve kategori seçildiğinde,
 * kullanıcıya otomatik olarak mantıklı bir kurumsal e-posta ve 
 * rastgele karmaşık bir geçici şifre oluşturur.
 */
...
if (addUserForm) {
    /**
     * Firebase Auth REST API kullanarak kullanıcı oluşturur (Admin panelinden direkt auth oluşturma),
     * ve ardından Firestore `users` koleksiyonuna detaylı profil bilgilerini yazar.
     */
