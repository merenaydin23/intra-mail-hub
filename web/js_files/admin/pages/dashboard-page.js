...
const CHART_PALETTE = {
    ...
    accent: "#64748b",
    slate: "#1e293b",
    // Premium emerald-jade gradient (dark → light)
    emerald: ["#064e3b", "#065f46", "#047857", "#059669", "#10b981", "#34d399"],
    // Extended palette for large datasets
    emeraldWide: ["#064e3b", "#065f46", "#047857", "#059669", "#10b981", "#34d399", "#6ee7b7", "#a7f3d0", "#d1fae5", "#ecfdf5"],
    // Gradient function for bar charts
    barGradient: (ctx) => {
        if (!ctx.chart.chartArea) return '#059669';
...
function buildCharts(data) {
    ...
    activeCharts["regionChart"] = new Chart(document.getElementById("regionChart"), {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: CHART_PALETTE.emeraldWide.slice(0, data.sortedRegions.length),
                borderWidth: 2,
                borderColor: '#ffffff',
                cutout: '75%',
                hoverOffset: 8
    ...
    activeCharts["categoryChart"] = new Chart(document.getElementById("categoryChart"), {
        ...
        data: {
            ...
            datasets: [{
                data: [data.factory.length, data.regional.length, data.local.length],
    ...
    // Generate gradient colors per bar (dark to light based on rank)
    const companyColors = sortedCos.map((_, i) => {
        const palette = CHART_PALETTE.emeraldWide;
    ...
    activeCharts["companyChart"] = new Chart(document.getElementById("companyChart"), {
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    grid: { display: false },
                    ticks: { font: { size: 11, weight: '700' }, color: '#1e293b', padding: 8 }
                }
            }
        }
    });

    // ── Department Bar Chart (Horizontal) ──
    destroy("deptChart");
    const deptStats = {};
    ...
    const deptColors = sortedDepts.map((_, i) => {
        const shades = ['#064e3b', '#065f46', '#047857', '#059669', '#10b981', '#34d399'];
    ...
    activeCharts["deptChart"] = new Chart(document.getElementById("deptChart"), {
        ...
        options: {
            ...
            scales: {
                ...
                y: { grid: { display: false }, ticks: { font: { size: 11, weight: '700' }, color: '#1e293b', padding: 8 } }
            }
        }
    });
}
...
function buildMessageCharts(messages) {
    ...
    if (canvas) {
        ...
        gradientFill.addColorStop(0, 'rgba(16, 185, 129, 0.15)');
        gradientFill.addColorStop(0.5, 'rgba(16, 185, 129, 0.05)');
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        ticks: { stepSize: 1, font: { weight: '700', size: 11 }, color: '#64748b' }
                    },
                    x: { grid: { display: false }, ticks: { font: { weight: '700', size: 11 }, color: '#64748b' } }
                }
            }
        });
    }
    ...
    if (activeList) {
        if (!topSenders.length) activeList.innerHTML = '<p style="text-align:center; color:#94a3b8;">Veri yok.</p>';
            ...
            activeList.innerHTML = topSenders.map(([name, count], index) => {
                const rankColors = ['#064e3b', '#065f46', '#047857', '#059669', '#10b981'];
                const rankBg = ['#ecfdf5', '#ecfdf5', '#f0fdf4', '#f0fdf4', '#f0fdf4'];
                const rc = rankColors[index] || '#10b981';
                const rb = rankBg[index] || '#f0fdf4';
                return `
                <div style="display:flex; align-items:center; justify-content:space-between; padding:1rem; background:#fff; border:1px solid #f1f5f9; border-radius:18px; box-shadow:0 4px 6px -1px rgba(0,0,0,0.02); transition:all 0.2s;">
                    <div style="display:flex; align-items:center; gap:1rem;">
                        <div style="width:32px; height:32px; background:${rb}; border-radius:10px; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:0.8rem; color:${rc};">${index+1}</div>
                        <div style="font-weight:700; font-size:0.9rem; color:#1e293b;">${name}</div>
                    </div>
                    <div style="background:${rb}; color:${rc}; padding:4px 12px; border-radius:20px; font-size:0.75rem; font-weight:800;">${count} Mesaj</div>
                </div>
            `}).join("");
        }
    }
        ...
        if (sortedReg.length) {
            ...
            busyEl.innerHTML = `
                ...
                <div style="display:flex; align-items:center; gap:8px; margin-top:1rem;">
                    ...
                    <span style="font-size:0.9rem; color:#059669; font-weight:700;">${count} Aktif Personel</span>
                </div>`;
    ...
    if (oldEl) {
        if (sortedAge.length) {
            ...
            oldEl.innerHTML = `
                ...
                <div style="display:flex; align-items:center; gap:8px; margin-top:1rem;">
                    <div style="width:8px; height:8px; background:#047857; border-radius:50%;"></div>
                    <span style="font-size:0.9rem; color:#047857; font-weight:700;">${age} Yaşında</span>
                </div>`;
    ...
    list.innerHTML = upcoming.map(u => {
        ...
        const actionBtn = isActionable 
            ? `<button class="bday-send-btn" data-id="${u.id}" data-name="${u.name}" data-surname="${u.surname}" 
                style="background:linear-gradient(135deg, #008B8B, #006666); color:white; border:none; padding:8px 16px; border-radius:12px; font-size:0.75rem; font-weight:800; cursor:pointer; transition:0.2s; display:flex; align-items:center; gap:6px;">
        ...
        return `
            <div style="display:flex; align-items:center; gap:1.25rem; padding:1.25rem; background:white; border:1px solid #f1f5f9; border-radius:20px; box-shadow:0 4px 6px -1px rgba(0,0,0,0.02); transition:0.2s;">
                <div style="width:48px; height:48px; background:#e6f3f3; border-radius:14px; display:flex; align-items:center; justify-content:center; color:#008B8B; font-weight:800; font-size:1rem; border:1px solid rgba(0,139,139,0.1);">${u.name[0]}${u.surname[0]}</div>
                <div style="flex:1;">
                    ...
                    ${isToday ? '<div style="color:#008B8B; font-size:0.7rem; font-weight:900; margin-top:4px; text-transform:uppercase; letter-spacing:0.05em;">🎂 Bugün Kutlanıyor</div>' : ''}
                    ${isTomorrow ? '<div style="color:#008B8B; font-size:0.7rem; font-weight:900; margin-top:4px; text-transform:uppercase; letter-spacing:0.05em;">🎉 Yarın Kutlanacak</div>' : ''}
                </div>
                ${actionBtn}
            </div>
        `;
