...
function setupRealtimeMessages() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshotTimeout = setTimeout(() => {
            let rawMessages = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            
            // GROUP BROADCASTS AND DIRECT MESSAGE THREADS
            const grouped = [];
            const broadcastGroups = {};
            const threadGroups = {};
            
            // 1. First Pass: Detect Bulk Direct Messages (same sender, subject, minute)
            const bulkDirectGroups = {};
            rawMessages.forEach(m => {
                if (!m.type || m.type === 'direct') {
                    const timeMinute = m.timestamp?.seconds ? Math.floor(m.timestamp.seconds / 60) : 0;
                    const bulkKey = m.senderId + '_' + m.subject + '_' + timeMinute;
                    if (!bulkDirectGroups[bulkKey]) bulkDirectGroups[bulkKey] = [];
                    bulkDirectGroups[bulkKey].push(m);
                }
            });
            
            // 2. Second Pass: Build groups while preserving descending order
            rawMessages.forEach(m => {
                const isBroadcast = m.type === 'announcement' || m.type === 'broadcast' || m.isBroadcast;
                
                if (isBroadcast && m.subject) {
                    const dateStr = m.timestamp?.toDate ? m.timestamp.toDate().toDateString() : '';
                    const key = m.subject + '_' + dateStr;
                    if (!broadcastGroups[key]) {
                        m._groupedRecipients = [{ id: m.receiverId, name: m.receiverName || 'Bilinmeyen' }];
                        m._groupCount = 1;
                        broadcastGroups[key] = m;
                        grouped.push(m);
                    } else {
                        broadcastGroups[key]._groupedRecipients.push({ id: m.receiverId, name: m.receiverName || 'Bilinmeyen' });
                        broadcastGroups[key]._groupCount++;
                    }
                } else if (!m.type || m.type === 'direct') {
                    const timeMinute = m.timestamp?.seconds ? Math.floor(m.timestamp.seconds / 60) : 0;
                    const bulkKey = m.senderId + '_' + m.subject + '_' + timeMinute;
                    const bulkGroup = bulkDirectGroups[bulkKey];

                    if (bulkGroup && bulkGroup.length > 1) {
                        // Part of a bulk direct message
                        if (bulkGroup[0].id === m.id) { // Only add the first one to the feed
                            m._groupedRecipients = bulkGroup.map(g => ({ id: g.receiverId, name: g.receiverName || 'Bilinmeyen' }));
                            m._groupCount = bulkGroup.length;
                            grouped.push(m);
                        }
                    } else {
                        // Normal thread (document already contains its own replies)
                        grouped.push(m);
                    }
                } else {
                    grouped.push(m);
...
function showMessageDetail(msg) {
    ...
    if (receiverEl) {
        if (msg._groupCount > 1) receiverEl.textContent = `Toplu Gönderim (${msg._groupCount} Alıcı)`;
        else receiverEl.textContent = msg.receiverName || (msg.isBroadcast ? 'Toplu Gönderim' : '-');
    }
    ...
    if (msg._groupCount > 1) {
        const rCard = document.getElementById('receiverInfoCard');
        const showList = msg._groupedRecipients.slice(0, 10).join(', ');
        const plus = msg._groupCount > 10 ? ` ve ${msg._groupCount - 10} kişi daha...` : '';
        
        if (rCard) {
            const displayNames = msg._groupedRecipients.slice(0, 10).map(r => r.name || 'Bilinmiyor');
            rCard.innerHTML = `<div style="padding:1rem; color:var(--text); font-size:0.8rem; line-height:1.6; text-align:left;"><strong><i class="fa-solid fa-users" style="color:var(--brand);margin-right:6px;"></i>Alıcılar (${msg._groupCount}):</strong><br><br> • ${displayNames.join('<br> • ')}<br><em>${plus}</em></div>`;
        }
        
        const recEl = document.getElementById('detReceiver');
        if (recEl) recEl.innerHTML = `<span style="font-weight:700; color:var(--amber, #d97706);"><i class="fa-solid fa-users"></i> Toplu Gönderim (${msg._groupCount} Kişi)</span>`;
        
        // Inject recipient list directly into body so they don't have to hover
        const bodyEl = document.getElementById('detBody');
        if (bodyEl) {
            const originalBody = (msg.content || msg.lastMessage || '-').replace(/\n/g, '<br>');
            bodyEl.innerHTML = `
                <div style="margin-bottom:1.5rem; padding:1.25rem; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; font-size:0.85rem; color:#475569;">
                    <strong style="color:var(--brand-ink); display:block; margin-bottom:0.5rem;"><i class="fa-solid fa-users" style="color:var(--amber);"></i> Alıcı Listesi:</strong>
                    ${msg._groupedRecipients.slice(0, 10).map(r => r.name || 'Bilinmiyor').join(', ')}${plus}
                </div>
...
async function initBroadcast() {
    ...
    const broadcastRegion = document.getElementById('broadcastRegion');
    if (broadcastTarget && broadcastRegion) {
        broadcastTarget.addEventListener('change', (e) => {
            if (e.target.value === 'factory') {
                broadcastRegion.value = 'all';
                broadcastRegion.disabled = true;
                broadcastRegion.style.opacity = '0.5';
            } else {
                broadcastRegion.disabled = false;
                broadcastRegion.style.opacity = '1';
            }
        });
    }
