...
export function renderMessageFeed(container, messages) {
    ...
    container.innerHTML = messages.map((m) => {
        ...
        let badgeHtml = '';
        if (m._groupCount > 1) {
            badgeHtml = `<span style="background:#fef3c7; color:#d97706; font-size:0.68rem; font-weight:800; padding:2px 6px; border-radius:6px; display:inline-flex; align-items:center; gap:3px; margin-left:6px; vertical-align:middle;"><i class="fa-solid fa-users" style="font-size:0.65rem;"></i> ${m._groupCount} Alıcı</span>`;
        } else if (replyCount > 0) {
            badgeHtml = `<span style="background:var(--brand-soft); color:var(--brand); font-size:0.68rem; font-weight:800; padding:2px 6px; border-radius:6px; display:inline-flex; align-items:center; gap:3px; margin-left:6px; vertical-align:middle;"><i class="fa-solid fa-comments" style="font-size:0.65rem;"></i> ${totalCount}</span>`;
        }
