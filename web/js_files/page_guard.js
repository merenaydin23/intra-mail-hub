...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        if (userCategory === 'admin' || userSubRole === 'admin' || userRole === 'admin') {
            correctDestination = "/pages/admin/yonetim_mesajlar.html";
            destinationName = "Yönetici Paneli";
            if (currentPath.includes("/pages/admin/") || currentPath.includes("/pages/portals/")) {
                isAuthorized = true;
            }
        ...
        if (isAuthorized) {
            ...
            // Eğer Admin portal sayfasındaysa, "Yönetim Paneline Dön" butonu ekle
            if ((userCategory === 'admin' || userSubRole === 'admin' || userRole === 'admin') && currentPath.includes("/pages/portals/")) {
                const btnReturn = document.createElement('button');
                btnReturn.innerHTML = '<i class="fa-solid fa-shield-halved"></i> Yönetici Moduna Dön';
                btnReturn.style.position = 'fixed';
                btnReturn.style.bottom = '2rem';
                btnReturn.style.right = '2rem';
                btnReturn.style.zIndex = '9999';
                btnReturn.style.padding = '12px 20px';
                btnReturn.style.borderRadius = '12px';
                btnReturn.style.background = '#0f172a';
                btnReturn.style.color = '#fff';
                btnReturn.style.border = 'none';
                btnReturn.style.fontWeight = '800';
                btnReturn.style.boxShadow = '0 10px 25px rgba(0,0,0,0.3)';
                btnReturn.style.cursor = 'pointer';
                btnReturn.style.display = 'flex';
                btnReturn.style.alignItems = 'center';
                btnReturn.style.gap = '8px';
                btnReturn.style.transition = 'all 0.2s';
                btnReturn.onmouseover = () => { btnReturn.style.transform = 'translateY(-3px)'; btnReturn.style.background = '#1e293b'; };
                btnReturn.onmouseout = () => { btnReturn.style.transform = 'translateY(0)'; btnReturn.style.background = '#0f172a'; };
                btnReturn.onclick = () => window.location.href = '/pages/admin/yonetim.html';
...
// ── DARK MODE ENTEGRASYONU KALDIRILDI ──
// Kullanıcının isteği üzerine Dark Mode (Karanlık Tema) tamamen kaldırıldı.
// Sistem her zaman varsayılan (Light) temada çalışacak.
